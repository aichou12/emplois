...
<h2 class="mb-4">Liste des demandeurs avec diplôme</h2>

@if($avecDiplomeUsers->isEmpty())
        <div class="alert alert-warning">
            Aucun demandeur avec diplôme trouvé.
        </div>
    @else
<table class="table table-striped table-bordered table-hover"  id="mainUserTable">
    ...
    <tbody>
    @foreach($avecDiplomeUsers as $user)
    <tr>
                    <td>{{ $user->numberid }}</td>
            <td>{{ $user->username }}</td>
            <td>{{ $user->email }}</td>
   ...
   <td>
   <a href="{{ route('admin.editavecdiplome', $utilisateur->id) }}" class="btn btn-success">
