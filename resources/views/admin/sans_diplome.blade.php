...
<h2 class="mb-4">Liste des demandeurs sans diplôme</h2>

@if($sansDiplomeUsers->isEmpty())
    <p>Aucun demandeur sans diplôme trouvé.</p>
@else
<table class="table table-striped table-bordered table-hover"  id="mainUserTable">
    ...
    <tbody>
    @foreach($sansDiplomeUsers as $user)
                    <tr>
                    <td>{{ $user->numberid }}</td>
            <td>{{ $user->username }}</td>
            <td>{{ $user->email }}</td>
   ...
   <td>
   <a href="{{ route('admin.editsansdiplome', $utilisateur->id) }}" class="btn btn-success">
